...
import com.example.retaildemo.service.UsersService;
import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import com.example.retaildemo.utils.MD5Utils;
...
//                String md5Pwd = MD5Utils.md5(pwd);
    ...
    @Override
    public ResultVO changeInfo(String name,Integer id, String pwd) {
        synchronized (this) {
            //1.根据用户查询，这个用户是否已经被注册
            QueryWrapper<Users> usersQueryWrapper = new QueryWrapper<>();
            usersQueryWrapper.eq("username",name) ;
            ...
            if (users == null) {
                ...
                int i = usersMapper.insert(user);
                if (i > 0) {
                    return new ResultVO(10000, "修改成功！", user);
                } else {
                    return new ResultVO(10001, "修改失败！", null);
                }
            } else {
                return new ResultVO(10001, "该用户名不可用，请更换！", null);
            }
        }
    }
    @Override
